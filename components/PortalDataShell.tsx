...
function ModuleIcon({ iconKey, size = 18 }: { iconKey?: string | null; size?: number }) {
  const Icon = (iconKey && moduleIcons[iconKey]) || LayoutDashboard;
  return <Icon size={size} />;
}
...
function MetricButton({
  ...
  return (
    <button className={`portal-game-stat-card ${tone || ""}`} onClick={onOpen} type="button">
      <Icon size={24} />
      <span>{title}</span>
      <strong>{value}</strong>
      <em>{status}</em>
...
function fallbackModules(dashboard: PortalDashboard): PortalModule[] {
  return [
    {
...
function fallbackActions(dashboard: PortalDashboard): PortalQuickAction[] {
  return [
    {
      ...
      action_path: "/portal",
      action_type: "link",
      description: "Return to portal home.",
      icon_key: dashboard.key,
      sort_order: 20,
    },
  ];
...
export function PortalDashboardDataShell({ dashboardKey }: { dashboardKey: string }) {
  ...
  async function loadLiveData() {
    ...
    setBootstrap(data);
    try {
      setRolePanel(await getPpdRoleOperationsPanel(supabase, data.dashboard_key || dashboardKey));
    } catch {
      setRolePanel(null);
    }
    if (["owner", "admin"].includes(data.dashboard_key || dashboardKey)) {
      try {
        setCommandCenter(await getPpdCommandCenter(supabase));
      } catch {
  ...
  return (
    <section className="portal-app-shell portal-live-shell portal-dashboard-redesign">
      ...
      <div className="portal-main-area">
        <div className="portal-topbar panel-card">
          <div>
            ...
            <h1>{isExecutive ? "Owner Command Center" : dashboardName}</h1>
        ...
        <section className="portal-game-command-center">
          <div className="portal-game-stat-grid">
            {isExecutive && commandCenter ? (
              <>
                <MetricButton title="Production Readiness" value={`${Math.round(Number(readiness.readiness_score || 0))}/100`} status={readiness.portal_ok ? "Live Operations" : "Needs Review"} Icon={Gauge} onOpen={() => setReviewPanel("readiness")} />
                <MetricButton title="Active Jobs" value={readiness.active_jobs ?? 0} status="Field workflow" Icon={PlaneTakeoff} onOpen={() => setReviewPanel("jobs")} />
                <MetricButton title="Verified Pilots" value={readiness.verified_pilots ?? 0} status="Ready pilots" tone="green" Icon={UsersRound} onOpen={() => setReviewPanel("pilots")} />
                <MetricButton title="AI Approvals" value={readiness.pending_ai_approvals ?? 0} status="Review queue" tone="amber" Icon={Rocket} onOpen={() => setReviewPanel("approvals")} />
                <MetricButton title="Critical Events" value={readiness.open_high_or_critical_events ?? 0} status="Attention" tone="red" Icon={ShieldAlert} onOpen={() => setReviewPanel("events")} />
              </>
            ) : (
              <MetricButton title={dashboardName} value={notifications.length} status="Live items" Icon={Gauge} onOpen={() => setReviewPanel("readiness")} />
            )}
          </div>

          <article className="panel-card portal-game-map-card portal-main-map-card">
        ...
        {rolePanel && (
          <section className="portal-role-ops-board panel-card">
            ...
            <div className="portal-role-ops-card-grid">
              {(rolePanel.cards || []).map((card, index) => <article className="portal-role-ops-stat" key={`${card.title || "card"}-${index}`}><span>{card.title}</span><strong>{card.value ?? 0}</strong><em>{card.label}</em></article>)}
            </div>
          </section>
        )}
        ...
        <section className="portal-role-ops-board panel-card">
          <div className="portal-game-panel-head"><div><span className="section-kicker">Quick Access</span><h3>Company functions</h3></div><span>{actions.length}</span></div>
          <div className="portal-action-list">
            {actions.slice(0, 8).map((action) => (
              <Link href={action.action_path || dashboardPath} className="portal-action-row" key={action.action_key}>
                <ModuleIcon iconKey={action.icon_key} size={18} /><div><strong>{action.action_label}</strong>{action.description && <span>{action.description}</span>}</div>
              </Link>
            ))}
          </div>
        </section>
        ...
        {reviewPanel && (
          <aside className="panel-card portal-review-drawer">
            <div className="portal-game-panel-head"><div><span className="section-kicker">Open Review</span><h3>{reviewPanel}</h3></div><button className="ghost-btn compact-portal-btn" type="button" onClick={() => setReviewPanel(null)}>Close</button></div>
            <p>Production readiness: {Math.round(Number(readiness.readiness_score || 0))}/100</p>
            <p>Active jobs: {readiness.active_jobs ?? 0}</p>
            <p>Verified pilots: {readiness.verified_pilots ?? 0}</p>
            <p>AI approvals: {readiness.pending_ai_approvals ?? 0}</p>
            <p>Critical events: {readiness.open_high_or_critical_events ?? 0}</p>
          </aside>
        )}
      </div>
    </section>
